...
def load_model_from_config(config, ckpt, verbose=True, copy_encoder = True):
    
    f = open("state_dict_log.txt", "w")
    f2 = open("model_param_log.txt", "w")
    ...
    for name, param in model.named_parameters():
        print(f"{name}: {param.shape}", file = f2)
        if "structcond_stage_model" in name:
            name_ = name.replace("structcond_stage_model", "model.diffusion_model")
            if name_ in sd:
                sd[name] = sd[name_].clone()
                print(name, "new", name_, "old")
    ...
    if len(m) > 0 and verbose:
        ...
        print("missing keys:")
        print([i for i in m if "spade" not in i])
...
def main():
    ...
    parser.add_argument(
        "--config",
        type=str,
#         default="/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/configs/stable-diffusion/v1-inference.yaml",
        default="/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/configs/stable-diffusion/v1-inference-structcond.yaml",
...
#         default = "/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/checkpoints/2025-02-23 17-55-30_ucf101_20000.ckpt",
        ...
        # default="/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/checkpoints/sd1.2/2025-02-24-10-28-55_ucf101_0.ckpt",
...
#     batch_train_prompts = list(df.loc[np.random.randint(df.shape[0], size = 8)]['prompt'])
    batch_train_prompts = list(df.loc[[0, 2000, 4000, 6000, 8000, 10000, 12000, 14000]]['prompt'])
    print(batch_train_prompts)
    ...
    x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
    x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
    np.save(f"/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/testStableSR.npy", x_samples_ddim)
