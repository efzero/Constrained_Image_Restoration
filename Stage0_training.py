...
#     seed_everything(opt.seed) ####unseed this for training
    ...
    for itrs in range(iterations):
        ...
        sc_ = None
        hr_list = []
        selected_im_inds = np.random.randint(df.shape[0], size = 8)
        ...
        if itrs % 100 == 0:
            ...
            x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
            x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
            np.save(f"/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/data/ucf101/train/intermediates{itrs}.npy", x_samples_ddim)
