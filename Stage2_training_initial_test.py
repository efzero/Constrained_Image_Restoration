...
#     seed_everything(opt.seed) ####unseed this for training
    ...
    for itrs in range(iterations):
        ...
        dt = torch.randint(-20, 20, (x.shape[0],), device = device)
        
        train_t = torch.clamp(t*20 + dt, 0, 999)
        ...
        ddim_inv = ddim_invs[t][0]
        
        code =  ddim_inv - clean_coef * z_prior ###multiplied by noise coef
...
#         code = code / noise_coef ###normalized to N(0,1)
        ...
        code = code / code.std() ###alternative normalization
...
#         code = randseed
        ...
        if itrs % 10 == 0:
            ...
            np.save(f"data/ucf101/train/Stage2_intermediates{itrs}_CIStraining{C0test}_rand_corrected_linearincrease.npy", x_samples_ddim)
            ...
            x_samples_ddim = torch.clamp((x_samples_ddim + 1.0) / 2.0, min=0.0, max=1.0)
            x_samples_ddim = x_samples_ddim.cpu().permute(0, 2, 3, 1).numpy()
            np.save(f"data/ucf101/train/Stage2_intermediates{itrs}_CIStraining{C0test}_corrected_linearincrease.npy", x_samples_ddim)
            
        #######################################Training##################################################
        loss,loss_dict = model.forward(z, c, t = train_t, sc = None, noise = final_noise) ####CCS Training
#         loss,loss_dict = model.forward(z, c, sc = None)  ####pure fine tuning
        loss.backward()
        opt_.step()
        opt_.zero_grad()
...
#         if itrs % 2000 == 0:
#             torch.save({'iterations':itrs,'state_dict': model.state_dict()}, "/scratch/liyues_root/liyues/shared_data/bowenbw/Constrained_Image_Restoration/checkpoints/sd1.5/" + ymdh + "_ucf101" + f"_{itrs}.ckpt")
