...
class LatentDiffusion(DDPM):
    ...
    def forward(self, x, c, t= None, xt = None, sc = None, y = None, adaptor = None, *args, **kwargs):
        if t is None:
            t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key is not None:
        ...
        return self.p_losses(x, c, t, xt = xt, sc = sc, y = y, *args, **kwargs)
...
#     @torch.no_grad() ##########disable this for training and lpips loss
    def apply_model(self, x_noisy, t, cond, sc = None, y = None, adaptor = None, return_ids=False, key = 'default'):
        
        if key != 'default':
            self.model.conditioning_key = key
        ...
        if self.model.conditioning_key in ['hybrid', 'hybrid_adm']:
            cond = {'c_crossattn': cond["c_crossattn"][0], 'c_concat': cond["c_crossattn"][1]}
            
        if self.model.conditioning_key in ['priorfusion', 'priorfusion_new']:
            cond = {'c_crossattn': cond["c_crossattn"][0], 'c_concat': cond["c_crossattn"][1]}
        ...
        if hasattr(self, "split_input_params"):
            ...
            x_recon = self.model(x_noisy, t, sc = sc, y = y, **cond, adaptor = adaptor)
    ...
    def p_losses(self, x_start, cond, t, xt = None, sc = None, y = None, adaptor = None, noise=None):
        noise = default(noise, lambda: torch.randn_like(x_start)) ######will the forward process change as the conditioning changes?
        if xt is None:
            x_noisy = self.q_sample(x_start=x_start, t=t, noise=noise)
        else:
            print(xt.shape, "xt")
            x_noisy = xt 
        model_output = self.apply_model(x_noisy, t, cond, sc = sc, y = y, adaptor = adaptor)
        loss_dict = {}
    ...
    def p_mean_variance(self, x, c, t,clip_denoised: bool, sc = None, y = None, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None):
        t_in = t
        model_out = self.apply_model(x, t_in, c,sc = sc, y = y, return_ids=return_codebook_ids)
    ...
    @torch.no_grad()
    def p_sample(self, x, c, t, sc = None, y = None, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance(x=x, c=c, t=t, sc = sc, y = y, clip_denoised=clip_denoised,
    ...
    @torch.no_grad()
    def progressive_denoising(self, cond, shape, verbose=True, callback=None, sc = None, y = None, quantize_denoised=False,
        ...
        for i in iterator:
            ...
            img, x0_partial = self.p_sample(img, cond, ts, sc = sc, y = y,
                                            clip_denoised=self.clip_denoised,
    ...
    def p_sample_loop(self, cond, shape, return_intermediates=False,
                      ...
                      mask=None, x0=None, img_callback=None, start_T=None,sc = None, y = None, 
                      log_every_t=None):
        ...
        for i in iterator:
            ...
            img = self.p_sample(img, cond, ts,sc = sc, y = y, 
                                clip_denoised=self.clip_denoised,
                                quantize_denoised=quantize_denoised)
            if mask is not None:
                img_orig = self.q_sample(x0, ts)
    ...
    @torch.no_grad()
    def sample(self, cond, batch_size=16, sc = None, y = None, return_intermediates=False, x_T=None,
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm', 'height3D', 'hybrid_adm', 'priorfusion', 'priorfusion_new']

    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, sc = None, y = None, adaptor = None, struct_cond=None, seg_cond=None):
...
#             out = self.diffusion_model(x, t, context=cc)
        elif self.conditioning_key == 'priorfusion_new':
            cc = c_crossattn
            ###################################large model     ##########################
            if c_concat.shape[1] > 4: ####prior enforcement
                #####non physical###################################################
#                 hr_prior, lr = c_concat[:,4:, :,:], c_concat[:,:4,:,:]
#                 xc = torch.cat((x, hr_prior), dim = 2)   ############################2x4x128x64
#                 lr_cond = torch.cat((lr, lr.clone()), dim = 2)   ############################2x4x128x64
                
                
                ####physical distribution
                hr_prior, lr = c_concat[:,4:, :,:], c_concat[:,:4,:,:]
                xc = torch.cat((x, lr), dim = 2)
                lr_cond = torch.cat((hr_prior, lr.clone()), dim = 2)
            else:
                xc = x ######2x4x64x64
                lr_cond = c_concat  ########2x4x64x64
#             xc = torch.cat((x, c_concat), dim = 1)
            xc = torch.cat((xc, lr_cond), dim = 1) ############################2x8x128x64
#             print(xc.shape, "xcshape")
            out = self.diffusion_model(xc, t, sc = sc, context = cc)
    
            if c_concat.shape[1] > 4:
...
#             print(out.shape, "out shape")
        ...
        elif self.conditioning_key == 'priorfusion':
            cc = c_crossattn
            xc = torch.cat((x, c_concat), dim = 1)
            out = self.diffusion_model(xc, t, sc = sc, context = cc) 
            ...
            # print(x.shape, "x", c_concat.shape, "c_concat")
        ...
        elif self.conditioning_key == 'hybrid_adm':
#             xc = torch.cat([x] + c_concat, dim=1)
#             cc = torch.cat(c_crossattn, 1)
            # out = self.diffusion_model(xc, t, context=cc)
#             print(xc.shape, 'xc')
            xc = torch.cat((x,c_concat), dim=2)
            cc = c_crossattn
            out = self.diffusion_model(xc, t, sc = sc, y = y, context = cc)
            out1 = out[:,:,:x.shape[2],:]
            out = out1
            # out2 = out[:,:,24:,:]
            # out = (out1 + out2)/2
        ...
        elif self.conditioning_key == 'height3D':
            cc = torch.cat(c_crossattn, 1)
            out = self.diffusion_model(x, t, sc = sc, context=cc)
        else:
            raise NotImplementedError()
...
#         print(type(struct_cond), struct_cond.keys())
